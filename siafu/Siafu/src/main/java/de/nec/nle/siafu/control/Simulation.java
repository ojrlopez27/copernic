...
public class Simulation implements Runnable {
	 ...
	 */
	public Simulation(final String simulationPath, final Controller control, HashMap<String, Class> map) {
		this.listeners = new ArrayList();
		this.simData = SimulationData.getInstance(simulationPath, map);
		this.siafuConfig = control.getSiafuConfig();
		this.simulationConfig = simData.getConfigFile();
		this.control = control;

		World.setShouldPrefillCache(true); //control.getSiafuConfig().getBoolean("ui.gradientcache.prefill"));
		World.setCacheSize(100); //control.getSiafuConfig().getInt("ui.gradientcache.size"));
	...
	public void run() {
		...
		this.outputPrinter =
				createOutputPrinter("csv"); //siafuConfig.getString("output.type"));
	...
	public synchronized void setPaused(final boolean state) {
		if (control.isGuiUsed()) {
			...
			for(SimulationListener listener : listeners) {
				listener.onStateChanged("pause", state);
			}
		}
	}
